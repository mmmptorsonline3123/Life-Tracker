...
agent_communication:
  ...
  - agent: "main"
    message: "ADDITIONAL FIXES: (5) Transcription 422 bug root-fixed: native path now uses FileSystem.readAsStringAsync(base64) + JSON POST instead of FileSystem.uploadAsync multipart. Backend updated to handle both JSON base64 (native) and multipart (web). (6) Backend multipart handler bug fixed (str vs UploadFile). (7) WakeWordIndicator component added to Home screen: pulsing green dot with two staggered ring animations, slides in/out when wakeMode changes, shows 'Listening/Recording/Processing' state. Component has testID='wake-indicator'."